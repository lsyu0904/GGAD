...
from utils import test_recon, test_sage, load_data, pos_neg_split, normalize, pick_step, test_aegis
from model import PCALayer
from layers import InterAgg, IntraAgg
from graphsage_aegis import *
...
class ModelHandler(object):
    ...
    def train(self):
        ...
        agg_gcn = GCNAggregator(features, feat_data, cuda=args.cuda)
        ...
        for epoch in range(args.num_epochs):
            ...
            sampled_idx_train = idx_train + idx_test
            ...
            for batch in tqdm(range(num_batches)):
                ...
                optimizer.zero_grad()
                if args.cuda:
                    loss_g, loss_gen = gnn_model.loss(batch_nodes)
                else:
                    loss_g, loss_gen = gnn_model.loss(batch_nodes)
            ...
            print(f'Epoch: {epoch}, loss_g: {loss_g_sum / num_batches}, loss_gen: {loss_gen_sum / num_batches}, time: {epoch_time}s')

            # Valid the model for every $valid_epoch$ epoch
            if epoch % args.valid_epochs == 0:
                print("Valid at epoch {}".format(epoch))
                test_aegis(idx_valid, y_valid, gnn_model, args.batch_size, args.thres)
